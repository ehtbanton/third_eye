...
/// TURBO colormap for depth map visualization.
/// Based on Google's Turbo colormap - better than JET for depth perception.
/// Reference: https://research.google/blog/turbo-an-improved-rainbow-colormap-for-visualization/
class TurboColormap {
  /// 256-entry TURBO colormap lookup table [R, G, B]
  static const List<List<int>> _turboLUT = [
  ...
  /// Apply TURBO colormap to disparity values and return RGBA bytes.
  ///
  /// [disparity] - List of disparity values (any range)
  /// [width] - Image width
  /// [height] - Image height
  /// Returns RGBA bytes (length = width * height * 4)
  static Uint8List apply(List<double> disparity, int width, int height) {
    if (disparity.isEmpty) {
      return Uint8List(0);
    }

    // Find min/max for normalization
    double minVal = disparity[0];
    double maxVal = disparity[0];
    for (final v in disparity) {
    ...
    for (int i = 0; i < disparity.length && i < width * height; i++) {
      // Normalize to 0-255
      int idx = ((disparity[i] - minVal) / range * 255).clamp(0, 255).toInt();

      // Lookup TURBO color
      final color = _turboLUT[idx];

      // Write RGBA
      output[i * 4 + 0] = color[0]; // R
      output[i * 4 + 1] = color[1]; // G
      output[i * 4 + 2] = color[2]; // B
  ...
  /// Apply TURBO colormap to normalized [0-1] disparity values.
  static Uint8List applyNormalized(List<double> normalizedDisparity, int width, int height) {
    final output = Uint8List(width * height * 4);

    for (int i = 0; i < normalizedDisparity.length && i < width * height; i++) {
      int idx = (normalizedDisparity[i] * 255).clamp(0, 255).toInt();
      final color = _turboLUT[idx];
  ...
  /// Get a single TURBO color for a normalized value [0-1].
  static List<int> getColor(double normalizedValue) {
