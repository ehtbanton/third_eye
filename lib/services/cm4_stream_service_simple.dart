...
/// Simplified CM4 Stream Service - connects directly via static IP
/// No BLE required - CM4 connects to phone's hotspot
class Cm4StreamService {
  // CM4 static IP on phone's hotspot network
  ...
  void _initialize() {
    for (var camera in CameraFeed.values) {
      _imageControllers[camera] = StreamController<Uint8List>.broadcast();
      _stateControllers[camera] = StreamController<Cm4ConnectionState>.broadcast();
  ...
  /// Connect to all CM4 cameras at static IP
  Future<void> connectAll() async {
    debugPrint('CM4StreamService: Connecting to all cameras at $cm4StaticIp');
  ...
  Future<void> connect(CameraFeed camera) async {
    final port = cameraPorts[camera]!;
    final url = 'http://$cm4StaticIp:$port/stream';
  ...
  /// Health check - ping CM4 to see if it's reachable
  Future<bool> healthCheck() async {
    try {
      final url = 'http://$cm4StaticIp:8081/health';
      final response = await http.get(Uri.parse(url)).timeout(
        const Duration(seconds: 3),
      );
      return response.statusCode == 200;
    } catch (e) {
      debugPrint('CM4StreamService: Health check failed: $e');
      return false;
    }
  }
